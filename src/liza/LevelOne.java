...
public class LevelOne extends BasicGameState{
	...
	Animation lisa, ghostUp, ghostDown, ghostLeft, ghostRight;
	...
	public LevelOne(int levels){
		
	}

	@Override
	public void init(GameContainer gc, StateBasedGame sbg) throws SlickException {
		System.out.println("Level One Ready!");
	...
	public void update(GameContainer gc, StateBasedGame sbg, int delta) throws SlickException {
		...
		if(playerMoves>0){
				...
				if(calcPoints>0){
					if(input.isKeyPressed(Input.KEY_DOWN)){
						array = Player.move(posPlayer, Input.KEY_DOWN, upDownRmae, leftRightRmae, man, manDown, manUp, manLeft, manRight);
						if(array[1] == 1){
							posPlayer = array[0];
							man = manDown;
							calcPoints -= 3;
				...
				if(calcPoints>0){
					if(input.isKeyPressed(Input.KEY_UP)){
						array = Player.move(posPlayer, Input.KEY_UP, upDownRmae, leftRightRmae, man, manDown, manUp, manLeft, manRight);
						if(array[1] == 1){
							posPlayer = array[0];
							man = manUp;
							calcPoints -= 3;
				...
				if(calcPoints>0){
					if(input.isKeyPressed(Input.KEY_RIGHT)){
						array = Player.move(posPlayer, Input.KEY_RIGHT, upDownRmae, leftRightRmae, man, manDown, manUp, manLeft, manRight);
						if(array[1] == 1){
							posPlayer = array[0];
							man = manRight;
							calcPoints -= 3;
				...
				if(calcPoints>0){
					if(input.isKeyPressed(Input.KEY_LEFT)){
						array = Player.move(posPlayer, Input.KEY_LEFT, upDownRmae, leftRightRmae, man, manDown, manUp, manLeft, manRight);
						if(array[1] == 1){
							posPlayer = array[0];
							man = manLeft;
							calcPoints -= 3;
		...
		if(lisaMoves>0){
			if(!(posLisa==posPlayer)){
				posLisa = Lisa.move( heuristic, posLisa, tileNames, upDownRmae, leftRightRmae);
				lisaMoves--;
				if(lisaMoves == 0){
					playerMoves = 1;
				}
